  ...
  return (
    <div className=" max-w-5xl mx-auto  my-18 p-5  border rounded-2xl overflow-hidden text-white">
      <div className=" flex gap-1 items-end py-5">
        <h1 className="  uppercase text-4xl md:text-6xl font-semibold">
          todolist
        </h1>
      ...
      <div className="  my-10 p-10 rounded-2xl bg-[#343a40]  ">
        ...
        <p className="text-xl text-gray-400">
          {tacheIncomplete > 0
            ? `Vous avez ${tacheIncomplete} tâche(s) en cours`
            : `Vous n'avez aucune tâche en cours,Ajouter en une !`}
        </p>
        <div className=" my-10   ">
          <ul className=" flex flex-col gap-6">
            {tache.map((task) => (
              <li
                key={task.id}
                className={` rounded-full p-4 text-xl flex items-center justify-between shadow_box  ${task.completed ? "bg-green-400 " : undefined
                  } `}
              >
                <div className="flex items-center gap-1">
                  <span className="text-[18px] "> {task.id}. </span>
                  <p
                    className={` ${task.completed ? "line-through text-white" : undefined
                      } `}
                  >
                    {" "}
                    {task.text}
                ...
                <div className="flex items-center gap-5">
                  <button
                    onClick={() => TacheComplete(task.id)}
                    className={`hover:scale-[0.9] duration-300  transition-all cursor-pointer `}
                  >
        ...
        <div className=" my-10 ">
          <p className=" text-2xl font-medium text-gray-400">
            {totalTachesAccomplie < 0
              ? `Vous avez accompli ${tacheComplete} tâche(s) sur ${tacheIncomplete}`
              : `${tacheIncomplete > 0
          ...
          {tache.length > 0 && (
            <div className="flex items-center justify-end  my-5 p-5">
              <button onClick={viderListe} className=" p-4 bg-red-600 rounded-full duration-300 transition-all hover:scale-90  cursor-pointer">Vider la liste</button>
            </div>
          )}
        </div>
